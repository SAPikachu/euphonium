...
const VERSION_FULL: &'static str = concat!(env!("CARGO_PKG_NAME"), " ", env!("CARGO_PKG_VERSION"),);

fn mioco_config_start_ex<F, T>(
    threads: usize,
...
pub fn main() {
    ...
    let config = Config::from_file(&args.flag_config).unwrap_or_else(|e| {
        debug!("Error: {:?}", e);
        error!(
