...
fn invoke_with_timeout<T, F, TRet>(
    inner: &mut T,
    rw: mioco::RW,
    timer: &mut Timer,
    mut try_fn: F,
...
impl TcpStreamExt for TcpStream {
    fn connect_with_timeout<TMs: AsTimeoutMs>(addr: &SocketAddr, timeout: TMs) -> io::Result<Self> {
        let mio_stream = (mio::tcp::TcpStream::connect(addr))?;
        let mut ret = MioAdapter::new(mio_stream);
        let mut timer = Timer::new();
        timer.set_timeout(timeout.as_timeout_ms());
        (invoke_with_timeout(&mut ret, mioco::RW::write(), &mut timer, |x| {
            x.take_socket_error().map(Some)
        }))?;
        Ok(ret)
    }
}

pub struct WithTimeoutState<T>
where
    T: Evented,
{
    inner: T,
    ...
    T: Evented,
{
    ...
    fn invoke<F, TRet>(&mut self, rw: mioco::RW, try_fn: F) -> io::Result<TRet>
    where
        F: FnMut(&mut T) -> io::Result<Option<TRet>>,
    {
        ...
        invoke_with_timeout(
            &mut self.inner,
            rw,
            self.timer.as_mut().or_else(|| timer.as_mut()).unwrap(),
            try_fn,
        )
    }
}
...
impl<T> io::Read for WithTimeoutState<MioAdapter<T>>
where
    T: 'static + mio::Evented + mio::TryRead,
{
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.invoke(mioco::RW::read(), move |x| x.try_read(buf))
    }
}
impl<T> io::Write for WithTimeoutState<MioAdapter<T>>
where
    T: 'static + mio::Evented + mio::TryWrite,
{
