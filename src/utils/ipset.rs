...
impl Debug for IpSet {
    fn fmt(&self, f: &mut Formatter) -> Result<(), FmtError> {
        write!(
            f,
            "IpSet ({} nodes, {} results)",
            self.table.mem_usage().0,
            self.table.mem_usage().1
        )
    }
}
impl IpSet {
    ...
    pub fn add_entry(&mut self, addr: IpAddr, masklen: u32) -> Result<(), Error> {
        match addr {
            IpAddr::V4(v4) => self.add_entry(IpAddr::V6(v4.to_ipv6_mapped()), masklen + 96),
            IpAddr::V6(v6) => {
                if masklen > 128 {
...
mod tests {
    ...
    fn test_ipset() {
        ...
        set.add_entry("2404:6800:4004:800::".parse().unwrap(), 64)
            .unwrap();
